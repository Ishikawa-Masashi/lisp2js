...
const lisp_module = function () {
      ...
      if (tag === 'def' || tag === ':=' || tag === 'set!' || tag === 'const') {
        ...
        if (l.rest.first instanceof $List) {
          ...
          if (isNaN(l.rest.first)) {
            // not a number
            o =
              l.rest.first[0] === '"' ? l.rest.first : '"' + l.rest.first + '"'; // if val is string, return itself; otherwise return "val"
        ...
        if (param_or_assignment) {
          ...
          o +=
            compiler(conseq, is_last_exp, is_recur, null, true, null) + ' : ';
  ...
  const compile = function (input_string: string, print_eval_result$?) {
